...
import { MBR } from './types.js';
import { t, setLanguage, updateUI } from './i18n.js';
...
document.getElementById('lang-select')?.addEventListener('change', (e) => {
  const lang = (e.target as HTMLSelectElement).value as 'ja' | 'en';
  setLanguage(lang);
});
// 地図の初期化 (国土地理院)
...
// MBRクリアと再描画関数
function drawMBRs(mbrList: MBR[]) {
  // ① 既存の描画済み図形を全てクリア
  mbrLayerGroup.clearLayers();

  if (!Array.isArray(mbrList) || mbrList.length === 0) return;

  const bounds: L.LatLngBounds[] = [];

  mbrList.forEach((mbr) => {
    // [minY, minX] (南西) と [maxY, maxX] (北東)
    const southWest: L.LatLngTuple = [mbr.minY, mbr.minX];
    const northEast: L.LatLngTuple = [mbr.maxY, mbr.maxX];
